 ...
 */
public class HttpResponseChannel1 implements Channel {
   ...
   */
  public HttpResponseChannel1(HttpServerConnection hsc) {
  ...
  public HttpResponseChannel1 setGZipCompressionEnabled(boolean bool) {
    gzip = bool;
    return this;
  }
  ...
  private void handleGet(HttpRequest req) throws IOException {
    HttpResponse resp = createResponse(new Transport());
    InputStream in = getClass().getResourceAsStream(
        "/us/pserver/revok/http/revok.html");
    try {
      resp.setEntity(new InputStreamEntity(in, in.available(), ContentType.TEXT_HTML));
      processor.process(resp, context);
      resp.removeHeaders("Content-Encoding");
      //resp.addHeader("Content-Encoding", "deflate");
      //printResponse(resp); System.out.println("* Done printing..."); if(true) return;
      conn.sendResponseHeader(resp);
      conn.sendResponseEntity(resp);
      conn.flush();
      //conn.close();
      System.out.println("* Done handleGet...");
    }
    catch(HttpException e) {
      throw new IOException(e.toString(), e);
    }
  ...
  private Transport handleJSRequest(HttpEntity entity) throws IOException {
    if(entity == null) return null;
    Map jsmap = JsonReader.jsonToMaps(EntityUtils.toString(entity));
    EntityUtils.consume(entity);
    RemoteMethod rm = new RemoteMethod();
    System.out.println("** HttpResponseChannel1.handleJSRequest: "+ jsmap);
    if(jsmap.containsKey("types")) {
      JsonObject types = (JsonObject) jsmap.get("types");
      Object[] os = types.getArray();
      System.out.println(" ** jsmap.get( types ): "+ Arrays.toString(os));
      System.out.println("  ** types[0]: "+ os[0]+ ", type: "+ os[0].getClass());
      JsonObject params = (JsonObject) jsmap.get("params");
      Object[] as = params.getArray();
      System.out.println(" ** jsmap.get( params ): "+ Arrays.toString(as));
      System.out.println("  ** params[0]: "+ as[0]+ ", type: "+ as[0].getClass());
    }
    rm.forObject(jsmap.get("object").toString())
        .method(jsmap.get("method").toString());
    return null;
  }
  ...
  public Transport read() throws IOException {
    ...
    try {
      ...
      printRequest(basereq);
      isGet = !HttpEntityEnclosingRequest.class
      ...
      if(request.getRequestLine().getUri().contains("/js")) {
        return handleJSRequest(content);
      }
  ...
  public void printRequest(HttpRequest req) {
    System.out.println("** HttpRequest: "+ req.getRequestLine()+ " **");
    Iterator it = req.headerIterator();
    while(it.hasNext()) {
  ...
  public void printRequest(HttpEntityEnclosingRequest req) throws IOException {
    System.out.println("** HttpRequest: "+ req.getRequestLine()+ " **");
    Iterator it = req.headerIterator();
    while(it.hasNext()) {
      System.out.println("    "+ it.next());
  ...
  public void printResponse(HttpResponse resp) throws IOException {
    System.out.println("** HttpResponse: "+ resp.getStatusLine()+ " **");
    Iterator it = resp.headerIterator();
    while(it.hasNext()) {
      System.out.println("    "+ it.next());
    }
    System.out.println();
