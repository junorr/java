...
/**
 * BeanReader utility class to generate SitAcss
 * JavaBean from a given java.sql.ResultSet.
 */
public class SitAcssReader {

  private final ResultSet rset;

  /**
   * Default constructor receiving the ResultSet.
   * @param rs ResultSet containing the SitAcss information.
   */
  public SitAcssReader( ResultSet rs ) {
    if(rs == null) {
  ...
  /**
   * Create a SitAcss bean with the ResultSet information.
   * @return The created SitAcss bean.
   */
  public SitAcss readBean() throws SQLException {
    SitAcss bean = new SitAcss();
    if(this.contains( "cd_sit_acss" )) {
      bean.setCdSitAcss( rset.getInt("cd_sit_acss") );
    }
    if(this.contains( "nm_sit_acss" )) {
      bean.setNmSitAcss( rset.getString("nm_sit_acss") );
    }
    return bean;
  }
