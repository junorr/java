...
final class Parser {
    ...
    private List<ParserListener> list;
    private boolean run;
    ...
    public void addParserListener(ParserListener pl) {
      if(pl != null) list.add(pl);
    }
    
    
    public List<ParserListener> listeners() {
      return list;
    }
    ...
    Object parse(int from,int to) throws IOException, FSException {
        ...
        getNextToken();
        while (tok.ttype!=LexAnn.TT_EOF && run) {
    ...
    private void setPrio(){
        if (opPrio==null){
            ...
            opPrio.put(LexAnn.TT_MINUS,prio);
            prio=new Integer(20);
    ...
    private void getNextToken() throws IOException {
        if (tok.ttype==LexAnn.TT_EOL) {
            if (code.getCurLine() < maxLine) {
                ...
                char[] cline = code.getLine();
                this.updateListeners(new String(cline));
                tok.setString(cline);
                tok.nextToken();
            } else {
