...
public interface DateTime {
  ...
  static class DateTimeImpl implements DateTime {
    ...
    DateTimeImpl(Date date) {
      this.date = new Date(Match.notNull(date).getOrFail().getTime());
    }
    
    
    DateTimeImpl(ZonedDateTime zd) {
      this.date = Date.from(Match.notNull(zd).getOrFail()
          .toInstant()
      );
    }
    ...
    DateTimeImpl(LocalDateTime ld) {
      this.date = Date.from(Match.notNull(ld).getOrFail()
    ...
    DateTimeImpl(Instant it) {
      this.date = Date.from(Match.notNull(it).getOrFail());
    }
    
    
    DateTimeImpl(DateTime dt) {
      this.date = new Date(Match.notNull(dt).getOrFail().toDate().getTime());
    }
