...
public class ResourceLoader {
  ...
  public static int DEFAULT_CALLER_LEVEL = 2;
   ...
   * @return Nova instância de ResourceLoader.
   */
  public static ResourceLoader caller(int callerStackLevel) throws ResourceLoadException {
    if(callerStackLevel < DEFAULT_CALLER_LEVEL) {
      String msg = String.format("Bad caller stack level: %d < %d", callerStackLevel, DEFAULT_CALLER_LEVEL);
    ...
    try {
      ...
      return of(cur.getContextClassLoader()
          .loadClass(els[Math.min(callerStackLevel, els.length-1)].getClassName())
      );
    } catch(Exception e) {
  ...
  public static ResourceLoader caller() throws ResourceLoadException {
    return caller(DEFAULT_CALLER_LEVEL);
  }
