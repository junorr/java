 ...
 * @author juno
 */
public class ConnectionsHandler implements KeepoutHandler {
  ...
  public void handleRequest(HttpServerExchange hse) throws Exception {
    ...
    hse.putAttachment(ATTACHMENT_SOCKET, sch);
  ...
  private String getTargetString(HttpServerExchange hse) {
    ...
    if(hse.getRequestHeaders().contains(TARGET)) {
      HexStringCoder hex = new HexStringCoder();
      String str = hex.decode(hse.getRequestHeaders().getFirst(TARGET));
  ...
  private Optional<String> getClientId(HttpServerExchange hse) {
    if(!hse.getRequestHeaders().contains(CLIENT_ID)) {
      badRequest(hse, "Client ID missing");
      return Optional.empty();
    }
    return Optional.of(hse.getRequestHeaders().getFirst(CLIENT_ID));
  }
