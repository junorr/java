...
public class Base64BufferCoder implements Coder<ByteBuffer> {
  ...
  private Base64ByteCoder bc;
  
  private ByteBufferConverter cv;
  
  
  public Base64BufferCoder() {
    bc = new Base64ByteCoder();
    cv = new ByteBufferConverter();
  }
  ...
  public ByteBuffer apply(ByteBuffer buffer, boolean encode) {
    if(buffer == null || buffer.limit() == 0)
      return buffer;
    return cv.reverse(bc.apply(cv.convert(buffer), encode));
  }
