...
public class DynamicBuffer {
  ...
  /**
   * Lê o conteúdo do buffer para o <code>ChannelBuffer</code>
   * informado.
   * @param ch <code>ChannelBuffer</code> onde será escrito
   * o conteúdo do buffer.
   * @return quantidade de bytes lidos, ou -1 caso
   * o argumento seja inválido.
   */
  public int read(ChannelBuffer ch) {
    if(ch == null || !ch.writable()) return -1;
    byte[] b = this.toArray();
    ch.writeBytes(this.toArray());
    return b.length;
  }
  ...
  /**
   * Escreve o conteúdo do <code>ChannelBuffer</code>
   * informado no buffer.
   * @param ch <code>ChannelBuffer</code> cujo conteúdo
