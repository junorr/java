...
import br.com.bb.disec.micro.util.CsvSender;
...
public class CsvDirectResponse implements DirectFormatResponse {
  ...
  public void doResponse(HttpServerExchange hse, ResultSet rst) throws Exception {
    ...
    CsvSender sender = new CsvSender(hse.getResponseSender());
  ...
  private void writeRow(CsvSender sender, ResultSet rst) throws SQLException {
  ...
  private void writeColumns(CsvSender sender, ResultSet rst) throws SQLException {
    ResultSetMetaData meta = rst.getMetaData();
    int cols = meta.getColumnCount();
    for(int i = 1; i <= cols; i++) {
