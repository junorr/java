...
public class MongoResultOp {
  ...
  public MongoResultOp(MongoCollection<Document> col) {
    if(col == null) {
  ...
  private MongoResultOp applyFilter(JsonObject json) {
    Document query = this.createFilterQuery(json);
    fhash = this.filterHash(query);
    String jhash = (json.has("filterHash") 
        ? json.get("filterHash").getAsString() : null
    );
    if(!fhash.equals(jhash)) {
      length = collection.count(query);
    }
  ...
  private MongoResultOp applySort(JsonObject json) {
    if(json.has("sortBy")) {
  ...
  private MongoResultOp applyLimit(JsonObject json) {
    if(json.has("limit")) {
