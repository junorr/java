...
import br.com.bb.disec.micro.client.*;
...
public class PublicCacheClient extends AbstractClient {
  ...
  private PublicCacheClient(String address, int port, String context) {
    super(address, port, context);
  }
  
  
  public static PublicCacheClient of(String address, int port, String context) {
    return new PublicCacheClient(address, port, context);
  }
  
  
  public static PublicCacheClient ofDefault() {
    return new PublicCacheClient(DEFAUTL_ADDRESS, DEFAULT_PORT, DEFAULT_CONTEXT);
  }
  ...
  public Optional<String> get(String key) throws IOException {
    ...
    System.out.println("* PublicCacheClient.request = "+ req.toString());
    HttpResponse hresp = req.execute().returnResponse();
    status = hresp.getStatusLine();
    Optional<String> opt = Optional.empty();
    System.out.println("* PublicCacheClient.status = "+ status.toString());
