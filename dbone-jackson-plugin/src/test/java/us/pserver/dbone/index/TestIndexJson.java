...
import us.pserver.dbone.serial.ExtendedSerializationService;
...
public class TestIndexJson {
  ...
  public void indexToJson() throws ClassNotFoundException, IOException {
    try {
      SerializationService serial = new JacksonSerializationService();
      serial = ExtendedSerializationService.builder(serial)
      ...
      Log.on("idx = %s", idx);
      Record<Index<Integer>> rec = Record.of(idx.region(), idx, serial);
      Log.on("rec = %s", rec);
      ByteBuffer buf = rec.toByteBuffer(ByteBuffer::allocate);
      Log.on("rec.toByteBuffer() = %s", buf);
      rec = Record.of(rec.getRegion(), buf, serial);
      Log.on("rec = %s", rec);
