...
public class RW3270
{
  ...
  private Cursor cursor;
    ...
    */
   public List<RW3270Field> getFields()
   {
      return fields;
   }
   ...
   {
      Enumeration e = Collections.enumeration(fields);
      ...
      e = Collections.enumeration(fields);
      while(e.hasMoreElements())
      {
         RW3270Field f = (RW3270Field)e.nextElement();
         if(!f.isProtected())
   ...
   {
   		return chars[cursor.position()].getField();
   }		
...
//			}
      ...
      cursor.increment(1);
      client.cursorMove(oldPos, cursor.position());
   }
   ...
   throws IsProtectedException
   {
      if(keyboardLocked)
         return;
      if(getChar(cursor.position()).getField().isProtected())
         throw new IsProtectedException();
      getChar(cursor.position()).setChar(' ');
      getChar(cursor.position()).getField().isModified(true);
      client.cursorMove(cursor.position(), cursor.position());
   }
   ...
   public void backTab()
   {
      if(keyboardLocked)
         return;
      int oldPos = cursor.position();
      cursor.setPosition(getPreviousUnprotectedField(cursor.position()));
      client.cursorMove(oldPos, cursor.position());
   }
   ...
   public void tab()
   {
      if(keyboardLocked)
         return;
      int oldPos = cursor.position();
      cursor.setPosition(getNextUnprotectedField(cursor.position()));
      client.cursorMove(oldPos, cursor.position());
   }
   ...
   throws IsProtectedException
   {
      if(keyboardLocked)
         return;
      if(getChar(cursor.position() - 1).getField().isProtected()||getChar(cursor.position() - 1).isStartField())
         throw new IsProtectedException();
      int oldPos = cursor.position();
      cursor.increment(-1);
      getChar(cursor.position()).getField().isModified(true);
  ...
  public void setCursor(int row, int pos) {
    cursor.setPosition(row, pos);
  }
  ...
  /**
   * Wait for the string 'search' apear on screen, or
   ...
   * otherwise.
   */
  public boolean waitString(String search, int timeout) {
    boolean found = false;
    long startTime = System.currentTimeMillis();
    long currTime = startTime;
    String screen = new String(this.display);
    found = screen.contains(search);
    
    while(!found) {
      synchronized(LOCK) {
        try { LOCK.wait(timeout * 1000); }
        catch(InterruptedException e) {}
      }
      screen = new String(this.display);
      found = screen.contains(search);
  ...
  /**
   * Wait for the field 'search' apear on the specified position, or
   * until reach the defined 'timeout';
   ...
   * otherwise.
   */
  public boolean waitField(int row, int col, String field, int timeout) {
    boolean found = false;
    long startTime = System.currentTimeMillis();
    long currTime = startTime;
    
    this.setCursor(row, col);
    RW3270Field fld = this.getField();
    if(fld == null) return false;
    String sfield = new String(fld.getDisplayChars());
    found = sfield.contains(field) || sfield.equalsIgnoreCase(field);
    ...
    while(!found) {
      ...
      fld = this.getField();
      if(fld == null) return false;
      sfield = new String(fld.getDisplayChars());
      found = sfield.contains(field) || sfield.equalsIgnoreCase(field);
      currTime = System.currentTimeMillis();
      long elapsed = currTime - startTime;
      if(elapsed >= (timeout * 1000))
  ...
  public String getString(Cursor c, int length) {
    if(c == null || length < 1) return null;
    return new String(this.display, c.position(), length);
  }
  
  
  public String getString(int row, int col, int length) {
    return this.getString(new Cursor(row, col), length);
  }
  
  
  public RW3270Field getFieldAt(Cursor cur) {
    if(cur == null) return null;
    RW3270Char chr = this.getChar(cur.position());
  ...
  public void setField(Object obj) {
    this.setField(cursor, obj);
  }
  
  
  public void setField(Cursor cur, Object obj) {
    if(obj == null || cur == null) return;
    RW3270Field fld = this.getFieldAt(cur);
    if(fld == null) return;
    String str = obj.toString();
    if(str.length() > fld.size())
      str = str.substring(0, fld.size());
    try { fld.setData(str); }
    catch(Exception e) {}
  }
  
  
  public void setField(int row, int col, Object obj) {
    this.setField(new Cursor(row, col), obj);
  }
  
  
  /**
   * Wait for the string 'search' apear on the specified position, or
   * until reach the defined 'timeout';
   * @param row Cursor row position
   * @param col Cursor column position
   * @param search string to search
   ...
   * otherwise.
   */
  public boolean waitString(int row, int col, String search, int timeout) {
    boolean found = false;
    long startTime = System.currentTimeMillis();
    long currTime = startTime;
    
    String str = this.getString(row, col, search.length());
    if(str == null) return false;
    found = str.contains(search) || str.equalsIgnoreCase(search);
    ...
    while(!found) {
      ...
      str = this.getString(row, col, search.length());
      if(str == null) return false;
      found = str.contains(search) || str.equalsIgnoreCase(search);
      currTime = System.currentTimeMillis();
      long elapsed = currTime - startTime;
   ...
   * otherwise.
   */
  public boolean waitField(Cursor c, String search, int timeout) {
    return this.waitField(c.getRow(), c.getColumn(), search, timeout);
  }
   ...
   private List<RW3270Field>     fields;//Vector of current fields
