...
public class IfExecutor extends Executor {
  ...
  public boolean exec(Command cmd, Session sess) {
    ...
    if(cmd.getArg(0) != null) {
      ...
      else if(val.startsWith(Command.STR_NOT)) {
        CommandParser cp = CommandParser.getInstanceFor(val);
      ...
      else if(CommandParser.isExpression(val)) {
        ExpressionParser p = new ExpressionParser(
            VarMemory.getInstance());
        Expression e = p.parse(val);
        if(e != null) val = e.resolve().toString();
  ...
  public static void main(String[] args) {
    IfExecutor ife = new IfExecutor();
    
    VarMemory.getInstance().put(new Var("verdade", "true"));
    String cmd = "if equals att at";
