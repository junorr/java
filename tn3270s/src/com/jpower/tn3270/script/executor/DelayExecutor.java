 ...
 */
public class DelayExecutor extends Executor {
  ...
  public boolean exec(Command cmd, Session sess) {
    ...
    if(cmd.getType() != CommandType.DELAY)
      return false;
    
    Var var = VarMemory.getInstance().get(cmd.getArg(0));
    if(var == null) var = Var.TEMP.setValue(cmd.getArg(0));
    int delay = var.getInt();
    if(delay == Var.INVALID_NUMBER)
      return false;
