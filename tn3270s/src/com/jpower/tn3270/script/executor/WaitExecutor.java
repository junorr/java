 ...
 */
public class WaitExecutor extends Executor {
  ...
  public boolean exec(Command cmd, Session sess) {
    ...
    if(cmd.getType() != CommandType.WAIT) 
      return false;
    ...
    sess.waitFor(new Cursor(row.getInt(), col.getInt()), 
        var.value(), Session.DEFAULT_TIMEOUT);
    return true;
  }
