...
public interface DateTime {
  ...
  static class DateTimeImpl implements DateTime {
    ...
    DateTimeImpl(ZonedDateTime zd) {
      this.date = Date.from(Objects.requireNonNull(zd).toInstant());
    }
    
    
    DateTimeImpl(LocalDateTime ld) {
      this.date = Date.from(Objects.requireNonNull(ld)
    ...
    DateTimeImpl(Instant it) {
      this.date = Date.from(Objects.requireNonNull(it));
    }
    
    
    DateTimeImpl(DateTime dt) {
      this.date = new Date(Objects.requireNonNull(dt).toTime());
    }
