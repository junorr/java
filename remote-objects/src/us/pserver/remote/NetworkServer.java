...
public class NetworkServer extends AbstractServer {
  ...
  private void preStart() {
    ...
    LogProvider.getLogger().info("Starting NetowrkServer...");
    running = true;
  ...
  public void run() {
    try(ServerSocket server = con.connectServerSocket();) {
      LogProvider.getLogger().info("Channel: "
          + factory.createChannel(new Socket()).getClass().getCanonicalName());
      LogProvider.getLogger().info("Listening on: "+ con.toString());
      LogProvider.getLogger().info("Server started!\n");
      ...
      LogProvider.getLogger().fatal("Error running");
      LogProvider.getLogger().fatal(e);
    }
    exec.shutdown();
  }
...
/*****************************************************************/
  ...
  public class SocketHandler implements Runnable {
    ...
    public SocketHandler(Channel ch, Socket sc) {
      ...
      LogProvider.getLogger().info("Handling socket: "+ sock.toString());
    }
    ...
    public void write(Transport trp) {
      ...
      try {
        channel.write(trp);
        LogProvider.getLogger().info("Response sent: "+ trp.getObject());
      } catch(IOException e) {
        LogProvider.getLogger().warn(
    ...
    public void run() {
      ...
      if(trp == null) {
        LogProvider.getLogger().info("Connection closed by client.");
        channel.close();
        return;
      }
